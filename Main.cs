...
namespace UKMisc
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        string[] Modifiers = { "MODIFIERS", "Back", "Toggle <color=orange>Sand</color> Mode", "Toggle <color=maroon>Vampire</color> Hook", "Toggle <color=red>Test</color> Mode" };
        bool[] _Modifiers = { false, false, false };
        ...
        public void OnGUI()
        {
            ...
            switch (ACT.name)
            {
                ...
                default:
                    ...
                    {
                        case true:
                            Text(35, "Sand Mode ON", new Rect(8, ratio-(Screen.height/2)+100, 1000, 1000));
                            break;
                        default:
                            break;
                    }
                    ...
                    {
                        case true:
                            Text(35, "Test Mode ON", new Rect(8, ratio-(Screen.height/2)+130, 1000, 1000));
                            break;
                        default:
                            break;
                    }
                    switch (_Modifiers[1])
                    {
                        case true:
                            ...
                            {
                                Text(35, "Vampire Mode ON", new Rect(8, ratio - (Screen.height/2)+160, 1000, 1000));
                                hookrdy = true;
                            }
                            break;
                        default:
                    ...
                    float nails = (int)GameObject.Find("Player").GetComponentInChildren<WeaponCharges>().naiAmmo;
                    switch (toggle)
                    {
                        case true:
                    ...
                    {
                        case true:
                            Text(35,"NAILS", new Rect(NailCounter.Value.x, NailCounter.Value.y-50f, 1000, 1000));
                            Text(60,nails.ToString(), new Rect(NailCounter.Value.x, NailCounter.Value.y, 1000, 1000));
                            break;
                    }
                    break;
            }
        ...
        private void Text(int fntsize,string txt, Rect Pos)
        {
            GUI.Label(new Rect(Pos.x+1f,Pos.y-1f,Pos.width,Pos.height),$"<size={fntsize}><color=black>{txt}</color></size>");
            GUI.Label(Pos,$"<size={fntsize}>{txt}</size>");
        }
        private void Warning()
        {
            Text(40,"<b>MUST HAVE MAJOR ASSISTS/CHEATS ACTIVATED</b>", new Rect(Screen.width / 4, Screen.height / 2, 1000, 100));
        }
